...
# neurons_count is not including the bias
# prev_neurons_count = number of neurons in the previous layer including the bias
layers = [ 
    # input layer
...
def forward_propagation(layers, x):
    ...
    for i, neuron in enumerate(layers[0].neurons[1:]): # omitting 1st neuron because it's 1 by default (bias)
...
class NeuralNetHolder:
    ...
    def predict(self, x):
        ...
        return forward_propagation(layers, x)
